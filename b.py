...
if __name__ == '__main__':
    ...
    with open('/data1/cth/nuscenes/val_img_night.txt','r') as f:
        lines = f.readlines()
        for line in lines:
            line = line.strip()
    ...
    for img in tqdm(img_clear_list):
        ...
        result_foggy_night_pth = '/data1/cth/nuscenes/foggy_night/'
        save_path = result_foggy_path + save_detail
        save_path_night = result_foggy_night_pth + save_detail
        ...
        L_atm = estimate_local_light_rf(clear_image_dark_channel,kernel_size=kernel_size)
        foggy_image = haze_linear2(clear_image,t_initial,L_atm)*255
        save_name = save_path + file_name
        save_name_night = save_path_night + file_name
        cv2.imwrite(save_name,foggy_image)
        cv2.imwrite(save_name_night,foggy_image)
